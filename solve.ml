...
let is_true_check : path_cond -> bool
  ...
  print_endline(string_of_int (is_true));
  if is_true=0 then true else false
...
= fun v1 v2_list ->
  ...
  | (s2, p2)::tl -> 
    match v1 with
    | (s1, p1) -> if is_true_check (PATHEQ (p1, p2))(*should modify*) then sym_val_check s1 s2 else solve_aux v1 tl
